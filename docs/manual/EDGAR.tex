...
\code{cik(query; by = :company | :ticker | :any)} & The rows matching
  ...
  case. \\
\code{full\_text\_search(query; forms, startdate, enddate, from)} &
...
The output looks cryptic because the SEC runs this on \texttt{Elasticsearch}, and the function returns its raw response envelope. Only the key \texttt{:hits} holds the results; the rest is search-engine plumbing:
...
Matches are under \code{.hits.hits}, and \code{.hits.total.value} is the total count.
