...
table} --- a \texttt{Vector} of \texttt{NamedTuple}s with fields \texttt{company},
\texttt{ticker} and \texttt{cik} (one record per ticker). \code{cik()} returns
...
\begin{itemize}[nosep]
  ...
  \item \code{:ticker} --- the exact (case-insensitive) ticker match
    (\code{0} or \code{1} row).
...
tables --- \texttt{DataFrames}, a \texttt{CSV} file, Arrow, a SQL database. A
company query may yield
...
julia> cik()[1:5]                   # first 5 of every company
# Output:
5-element Vector{@NamedTuple{company::String, ticker::String, cik::String}}:
...
\sig{full\_text\_search(query; forms = nothing, startdate = nothing, enddate = nothing, from = 0, size = 10)}

\noindent Search the \emph{contents} of filings (2001 onward) through the EDGAR
full-text search (EFTS) API, returning the parsed JSON. \code{forms} may be a
single string (\code{"10-K"}) or a collection; \code{startdate} and \code{enddate}
are \code{"YYYY-MM-DD"} strings; \code{from} and \code{size} page the results.
Matches are under \code{.hits.hits}, and \code{.hits.total.value} is the total
count.
