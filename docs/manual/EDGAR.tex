...
as a plain vector it indexes, filters and ingests straight into
\texttt{DataFrames}, \texttt{CSV}, Arrow and SQL sinks. A company query may yield
several rows, since one company can have multiple tickers (e.g. share classes
\code{GOOGL}/\code{GOOG}), although there is just one \texttt{company} and
\texttt{cik} among them.
\begin{lstlisting}
julia> cik("alphabet")              # by = :company is omitted since is the default
# Output:
...
julia> only(cik("AAPL"; by = :ticker)).cik   # bare CIK from a ticker
# Output:
"0000320193"
\end{lstlisting}
...
\noindent\code{full\_text\_search(query; forms = nothing, startdate = nothing, enddate = nothing, from = 0, size = 10)}
...
\noindent\code{fetch\_submissions(cik)}
...
\noindent\code{list\_recent\_filings(cik; count = 10)}
...
\noindent\code{download\_filing(cik, accession; destdir = ".")}
...
\noindent\code{parse\_filing(path)}
...
\noindent\code{extract\_section(html, names; max\_chars = 200\_000, base\_path = nothing)}
...
\noindent\code{save\_filing(text, metadata; outdir = "out")}
...
\noindent\code{company\_facts(cik)}
...
\noindent\code{company\_concept(cik, taxonomy, tag)}
...
\noindent\code{xbrl\_frames(taxonomy, tag, unit, period)}
...
\noindent\code{set\_user\_agent(user\_agent)}
...
\noindent\code{persist\_user\_agent(user\_agent; depot = first(DEPOT\_PATH))}
...
\noindent\code{unpersist\_user\_agent(; depot = first(DEPOT\_PATH))}
...
\noindent\code{set\_config(; cache, cache\_dir, cache\_ttl, cache\_max\_size, cache\_max\_age, host\_whitelist, allow\_file, user\_agent)}
...
\noindent\code{clean\_cache(max\_age\_seconds = CACHE\_TTL)}
...
\noindent\code{cache\_path\_for(url)}
...
\noindent\code{fetch\_url(url; use\_cache = true, timeout = 15, allow\_file = false)}
