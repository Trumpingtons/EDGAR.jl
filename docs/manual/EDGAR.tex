...
\subsubsection{full\_text\_search}

\sig{full\_text\_search(phrase; exact=true, forms=nothing, startdate=nothing, enddate=nothing, from=0)}

\noindent The function
\texttt{full\_text\_search} discovers \textit{filings} by their content (e.g. "which filings mention 'supply chain disruption'?"). Search the full text of SEC filings, everything filed from 2001 onward, for some \texttt{phrase} through the EDGAR
...
\begin{lstlisting}
julia> res = full_text_search("supply chain disruption"; forms = "10-K", startdate = "2026-01-01", )
...
\begin{lstlisting}
julia> filings_by_cik(320193; forms = "8-K", startdate = "2026-01-01").hits.total.value
5
\end{lstlisting}
...
without leading zeros. See \code{list\_recent\_filings} for a tidied view of just
the recent filings.

\subsubsection{list\_recent\_filings}

\sig{list\_recent\_filings(cik; count = 10)}

\noindent Return a filer's most recent filings as up to \code{count} rows of
\code{(accession, form, date)}, newest first. Built on \code{fetch\_submissions}.
